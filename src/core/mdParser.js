        ...
        return md
          .replace(
            ...
            (a, dotted, lang, content) => {
              ...
              let lines = [
                ...new DOMParser().parseFromString(contents, "text/html").body
                  .children,
              ];
              let lcount = 1;
              ...
              if (!lang && count < 3) {
                return `<kbd ${attrs || ""} class="${
                  classes || ""
                }">${content.trim()}</kbd>`;
              } else {
                return `<pre class="parse-code"><code class="number" lang="${lang.trim()}">${new Array(
                  lines.length
                )
                  .fill(0)
                  .map(
                    (l, i) =>
                      `<div class="token line">${
                        lines[i].innerHTML == "" ? "" : lcount++
                      }</div>`
                  )
                  .join(
                    ""
                  )}</code><code lang="${lang.trim()}">${contents}</code></pre>`;
              }
            }
          )
          .replace(
      ...
      convertedHTML = convertedHTML.map((x) => {
        if (/(\*+)([\s\S]+?)\*+/g)
          return x.replace(/(\*{1,3})([\s\S]+?)\*{1,3}/g, (a, $1, $2) => {
            ...
            return `${
              $1.length == 2
                ? `<em class="${classes || ""}" ${attrs || ""}>`
                : `<b class="${classes || ""}" ${attrs || ""}>${
                    $1.length == 3 ? `<em>` : ``
                  }`
            }${$2}${
              $1.length != 2 ? `</b>${$1.length == 3 ? `</em>` : ``}` : `</em>`
            }`;
          });
        else return x;
      });
